...
# 1. OpenSimplex 대신 C 기반 초고속 노이즈 snoise2 임포트
...
def get_biome(world: Any, x: float, y: float) -> Biome:
    ...
    동작 원리:
      - world에 temp_seed 및 moist_seed가 있으면
        snoise2 함수를 사용하여 4종 바이옴(fire, water, dirt, air) 중 하나를 반환합니다.
      - 노이즈 시드가 없으면 특별 바이옴(Biome.fifth_biome)을 반환합니다.

    좌표 처리:
      - 입력 좌표를 타일 중심으로 정렬한 뒤 노이즈 값을 조회합니다.
      - 노이즈값(temp, moist)이 양수인지 음수인지로 바이옴을 분류합니다.
    """
    # 2. 객체 유무 검사를 기존 'generator'에서 새로운 'seed' 변수명으로 변경
    has_temp = hasattr(world, "temp_seed")
    has_moist = hasattr(world, "moist_seed")
    ...
    if has_temp and has_moist:
        ...
        # 3. snoise2 함수 호출 방식으로 변경
        # snoise2(x, y, octaves=1, base=시드값) 구조로 연산합니다.
        temp = snoise2(
